...
- `処理日`, `申請処理日`, `連番`: date and legacy row-order structure.
...
- Opening balance uniqueness is **not enforced**. Legacy data contains multiple `入出区分CD=99` rows for some branches. Enforcing uniqueness now would lose or reject legacy-compatible data.
...
- `registered_*` and `updated_*` metadata are preserved/imported when present in source data.
